...
def j_function(c_R):
    ratio = (c_R / c_max)
    return F * K * np.sqrt(abs((ratio * (1 - ratio))))  ##################CHECK: ABS used to prevent complex numbers################
...
for i in range(no_timesteps):
    ...
    print('i_app is', i_app_temp, 'which is a', type(i_app_temp))
    c_temp = edge_conc_vals[i] 
    print('c_temp is', c_temp, 'which is a', type(c_temp))        
    j_temp = j_function(c_temp)
    print('j_temp is', j_temp, 'which is a', type(j_temp))
    if electrode.lower() == 'positive' or electrode == 'cathode':
        u_temp = U_function_pos(c_temp)
    else:
        u_temp = U_function_neg(c_temp)
    print('u_temp is', u_temp, 'which is a', type(u_temp))
    if j_temp == 0:
